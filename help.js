helpText=[
'INPUT',
'keyboard_check(_key)',
'keyboard_check_pressed(_key)',
'keyboard_check_released(_key)',
'mouse_check()',
'mouse_check_pressed()',
'mouse_check_released()',
'mouse_x and mouse_y is used for mouse/touch coordinates',
'wheelDir // is used for mouse wheel direction',
'MATH',
'abs(_value)',
'average(nums)',
'floor(_value)',
'round(_value)',
'irandom(_value)',
'random(_value)',
'sign(_value)',
'degtorad(_degree)',
'radtodeg(_degree)',
'lengthdir_x(_length, _direction)',
'lengthdir_y(_length, _direction)',
'bpmTimer(initialBpm, callback) //setBpm(newBpm) start() stop()',
'point_direction(_x1, _y1, _x2, _y2)',
'point_distance(_x1, _y1, _x2, _y2)',
'place_free(passableIDs, x, y, w, h, map);//if overlapping tiles ids not passable false',
'getOverlappingTiles(x, y, w, h, map, tileSize);// returns array of overlapping tiles values',
'easeIn(inp, target, rate)',
'easeOut(inp, target, rate)',
'lerp(inp, target, rate)',
'clamp(value, min, max)',
'inside(px, py, rx, ry, width, height, angle, originX, originY)',
'SOUNDS',
'make_sound(_src,buffers)',
'play(sound)',
'zzfx(...[1.5,.8,270,,.1,,1,1.5,,,,,,,,.1,.01]); // Piano',
'zzfxG();//caches sound',
'zzfxP();//plays cached sound',
'DRAWING',
'drawWrappedText(text, x, y, maxWidth, lineHeight,fontSize, weight)',
'text_fit(text,width)',
'draw_text(x, y, text, size, weight)',
'draw_centered_text(x, y, text, size, weight) ',
'draw_set_image(file)',
'draw_image(img,x,y,w,h,rot,ox,oy)',
'draw_image(img,x,y)',
'draw_image(img,x,y,w,h)',
'draw_image(img,x,y,w,h,rot,ox,oy,source_x,source_y,source_w,source_h)',
'drawTile(tId,x,y,w,h)',
'draw_circle(x, y, r, outline)',
'draw_line(x1, y1, x2, y2)',
'draw_rectangle(x, y, w, h,outline,rot,ox,oy)',
'draw_set_alpha(_alpha)',
'draw_set_color([r,g,b])',
'draw_set_linedash(dash)',
'draw_set_linewidth(width)',
'MISC',
'choose()',
'hide_mouse()',
'show_mouse()',
'fps is frames per second',
'prevents automatic redraw of canvas',
'run=false',
'deltatime for cpu speed adjustment',
'timeShift',
'MOUSE CLICK TO EXIT'
]
